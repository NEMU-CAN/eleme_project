...
class CartControllerTest {
    ...
    void cartLifecycle() throws Exception {
        ...
        mockMvc.perform(get(CART_URL).header("Authorization", auth()).param("businessId", "10001"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(1))
        ...
        mockMvc.perform(delete(CART_URL + "/" + cartId).header("Authorization", auth()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(1))
    ...
    void addWithDefaultQuantity() throws Exception {
        ...
        mockMvc.perform(post(CART_URL).header("Authorization", auth()).contentType(MediaType.APPLICATION_JSON).content(json))
                .andExpect(status().isOk())
    ...
    void deleteByFilter() throws Exception {
        ...
        mockMvc.perform(delete(CART_URL).header("Authorization", auth()).param("businessId", "10001").param("foodId", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(1))
                .andExpect(jsonPath("$.data").isEmpty());
    }
