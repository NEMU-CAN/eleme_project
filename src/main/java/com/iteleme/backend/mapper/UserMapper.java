...
public interface UserMapper {
    @Select("SELECT userId, password, userName, userSex, userImg, delTag FROM `user` WHERE userId = #{userId}")
    User findById(@Param("userId") String userId);

    @Select("SELECT userId, password, userName, userSex, userImg, delTag FROM `user` WHERE userId = #{userId} AND delTag = 1")
