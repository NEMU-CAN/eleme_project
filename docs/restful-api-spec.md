...
`food` 表已补充 `stock` 列（int，`NULL` 表示不限库存），接口层 `FoodVO` 返回 `stock`：

- 当前实现（轻量版）：仅展示——`GET /api/businesses/{id}/foods` 返回的每个食品带 `stock`；不做下单校验/扣减。
- 后续阶段：如需库存管理，可补充 `PATCH /api/v1/foods/{foodId}/stock`（见 §4.3.6）与下单扣减。
- 前端展示：统一读取接口返回的 `stock`，不要在页面中写死库存文案；`stock` 为 `NULL` 视为不限库存。
